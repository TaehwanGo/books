...
- 무언가 놓치고 있다고 생각할 때
  ...
  - 고객 행동은 검증할 만한 데이터가 있었나요?

- 기획 시작점에서 기술 또는 회사의 이익 관점이 아니라, '고객 관점'에서 고객에게 최고의 경험을 제공하고자 노력을 한다
...
- 사용자가 '아하 모먼트'를 경험 -> 재방문률(리텐션률)이 높아짐

#### 고객 여정 지도로 '아하 모먼트' 찾기

- 프로덕트를 사용하기 시작한 14일 이내의 신규 유저들을 자주 만나봐야 한다
- 활성(유지) 고객
  - 고객 여정을 들어보면 프로덕트를 처음 경험하는 동안 `공통적으로 무엇을 했는지` 알게 됩니다
    - 물론 데이터로 다시 검증을 해야 합니다
- 이탈 고객

  - 공통적으로 `무엇을 하지 않았는지` 알아보면 좋습니다
  - 만약 유지고객과 같은 행동을 했는데 이탈했다면, 그 이유를 찾아야 합니다
    - 이탈한 사용자와 같은 고객 여정을 반복해서 질문하고, 데이터로 다시 한번 확인하면 이탈 원인을 알 수 있게 됩니다

- 설문조사나 전화 인터뷰도 좋지만, 고객의 생생한 목소리를 들으며 감정과 행동을 관찰할 수 있는 화상 인터뷰나 오프라인 인터뷰를 추천합니다
- 인터뷰 시 질문
...
- 일찍 이탈한 사용자, 즉 '아하 모먼트'에 전혀 도달하지 못한 사용자에게 집중하는 것이 가장 중요합니다

  - 이탈한 고객이기 때문에 이들과 인터뷰를 잡기도 어렵고, 데이터가 없기 때문에 데이터 분석이 더 어렵습니다
  - 게다가 내가 함께 메이커들과 만든 프로덕트에 대해 쓴소리를 들어야 하니 인터뷰를 들으며 얼굴이 붉어지는 순간을 마주해야 하기도 합니다
  - 하지만 이들은 유지 고객과 같은 사용자 경험을 겪었으나, 우리 프로덕트가 유용하다고 생각하지 않은 사람들입니다
  - 이탈은 안타까운 일이지만, 무엇이 잘못되었는지 배울 수 있는 너무나 좋은 기회이기도 합니다

- 이탈 사용자들은 주로 우리 프로덕트가 그들이 원하는 프로덕트가 아니었거나, 경험의 장벽으로 인해 기대했던 가치를 찾지 못한 겁니다
  - 경험의 장벽인 경우 반드시 찾아서 제거해야 합니다

#### 데이터로 다시 한번 검증하기

- 고객 답변을 보하는 가장 좋은 방법: 관련 데이터와 결합하는 것
  - 이 전략은 고객 여정에 대한 전체 그림을 제공하고, 필요한 통찰력을 제공하는 데 도움을 줍니다
  - 따라서 프로덕트 데이터를 사용해 사용자가 언제 전환했고, 왜 전환했는지 이유를 알아봐야 합니다
  - 아하 모먼트를 포착하기 위해서 감이 아닌 데이터로 고객의 목소리를 한 번 더 검증해야 합니다
- 코호트 분석: 사용자 행동을 그룹으로 나눠 지표별로 수치화한 뒤 분석하는 기법
  - 획득, 행동으로 나뉩니다
    - 획득: 특정 시작날짜를 기준으로 사용자를 그룹화한 것
    - 행동: 유저가 앱 또는 웹사이트에서 수행한 특정 액션을 기반으로 사용자를 그룹화한 것
  - 아하모먼트는 행동 코호트 분석을 통해 찾을 수 있습니다
    - 특정 행동을 수행한 횟수 또는 기간에 따라 리텐션이 어떻게 변화하는지 확인합니다

#### 4단계로 행동 코호트 분석을 수행한 예시 - 크몽

- 1. 리텐션에 해당하는 활성화 고객 유지율 추적을 통해 고객 구매 여정의 `어떤 부분이 우리 프로덕트의 가치를 효과적으로 전달하는지 확인`할 수 있습니다
  - 크몽 예시: 리텐션이 높은 고객은 전문가에게 문의를 한 이후 빠른 시간 안에 긍정적인 답변을 전문가에게 받은 고객이었습니다
- 2. 반대로 이탈률을 측정해 `이탈한 사용자가 프로덕트에 지속적으로 시간을 투자`하는 데 충분한 가치를 느끼지 못하는 순간들을 찾아내고, 그 순간에 어떤 고객 경험의 장벽이 존재하는지, 제거해야 할 `고객 경험의 장애요소는 무엇인지 파악`할 수 있습니다
  - 크몽 예시: 전문가에게 첫 문의 메시지를 작성하기 전까지 신규 유저에게 강제했던 여러 퍼널들이 이 장벽에 해당했습니다
- 3. 그 다음으로 서비스의 주요 고객 행동(e.g. 주문)의 전환으로 이어지는 고객 행동을 찾기 위해 전환하지 않은 사용자와 데이터를 비교해봅니다
  - 전환한 사용자와 그렇지 않은 사용자 간의 주요 차이점을 여러 번의 데이터 분석을 통해 발견할 수 있습니다
  - 크몽 예시: 메시지를 보낸 이후에 전문가의 답변이 아에 없거나, 너무 늦은 경우
    - 반면 문의한 전문가에게 빠른 시간안에 긍정적인 답변을 받은 고객은 구매로 전환이 잘 이어졌습니다
- 4. 마지막으로 이 데이터를 바탕으로 '아하 모먼트'가 어디에 있는지 어느 정도 검증했다면, 리텐션을 높일 수 있는 사용자 행동 기반 가설을 세울 수 있게 됩니다

- 마지막으로 고객 데이터를 통해 고객 경험을 검증할 때는 다음과 같은 질문에 대한 답을 찾아보면 좋습니다
  - 고객이 구매로 전환할 때 온보딩 경험이 얼마나 중요한가요?
  - 우리 프로덕트가 제공하는 주요 기능과 상호 작용해 전환된 고객은 몇 명인가요?
  - 고객이 구매로 전환하기 전에 우리 프로덕트를 몇 번이나 다시 방문했나요?
...
- 아하모먼트를 찾아내는 체크 리스트
  - 고객 중심으로 디자인 되어 있는가?
  - 고객에게 정말로 도움이 되는 기능인가?
  - 원활한 사용자 경험을 제공하는가?
  - 데이터 분석을 통해 사용자의 행동을 분석하는가?
  - 사용자 교육을 제공하는가?
...
- 전략적인 방향이 일치하는지, 숫자 뒤에 숨겨진 진짜 의미는 무엇인지 생각해야 한다는 겁니다
...
- 프로세스
  - 목적 정의 - 예: 하반기 신규 프로덕트 개발 과제 선정을 위해
  - 대상 고객 선정 - 예: 우리 서비스 이탈 고객
  - 인터뷰 가이드 준비 - 예: 반정형 인터뷰 가이드 작성
  - 인터뷰 방법론 선택 - 예: 서비스 이탈 시점 팝업 레이어 사용
  - 인터뷰 실시 - 예: 실시간 응답 모니터링 및 기록
  - 데이터 분석 - 예: 텍스트 분석 및 패턴 인식
  - 인사이트 도출 - 예: 중고차 판매의 주요 망설임 요인 도출
  - 결과 적용 - 예: 하반기 개발 과제 적용
